...
export const getSellerLoginStats = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const loginStats = await User.aggregate([
      { $match: { role: 'seller' } }, // Match only sellers
      { $unwind: '$loginHistory' },
      ...
      { $sort: { '_id.sellerName': 1, '_id.year': 1, '_id.month': 1, '_id.day': 1, '_id.hour': 1, '_id.minute': 1 } }
    ]);
