...
export const createBundle = async (req: Request, res: Response) => {
  const { name, description, products, adminDiscount } = req.body;
  ...
  try {
    ...
    const discountAmount = (totalPrice * adminDiscount) / 100;
...
export const updateBundle = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (updateData.products) {
            ...
            const adminDiscount = updateData.adminDiscount || 0;
            const finalPrice = totalPrice - (totalPrice * adminDiscount / 100);
            ...
            bundle.adminDiscount = adminDiscount;
